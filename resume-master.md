# Larry Reynolds

## Contact

Larry Reynolds  
Principal Product Designer  

Portland, OR  
https://www.larryjr.design  
https://www.linkedin.com/in/larryjrdesign  
larry.jr.design@gmail.com  
503-475-7438

---

# Professional Positioning

## Strategic Systems Designer
Design leader focused on complex product ecosystems, workflow orchestration, AI-native product experiences, and human-centered systems thinking.

## Player / Coach
Hands-on design leader who combines strategic thinking, mentorship, collaboration, and executional craft.

## AI-Native Product Design
Explores how AI changes interaction models, onboarding, trust, workflow orchestration, and product behavior while maintaining strong human-centered principles.

---

# Target Roles

## Primary Targets
- Staff Product Designer
- Principal Product Designer
- Lead Product Designer

## Secondary Targets
- Product Design Lead
- Senior Staff Product Designer
- AI Product Designer
- Platform Product Designer

## Selective Leadership Roles
- Director of Product Design
- Head of Product Design
- Design Leadership

## Specialized Focus Areas
- AI-Native Product Design
- Systems Design
- Design Systems Leadership
- Platform Design
- Service Ecosystem Design
- Workflow Orchestration

---

# Core Positioning

Strategic product designer focused on systems thinking, AI-native workflows, and human-centered experiences. Experienced leading cross-functional initiatives across product, engineering, marketing, and data teams to transform fragmented platforms into cohesive, scalable ecosystems.

Combines UX strategy, interaction design, visual craft, design systems thinking, and functional prototyping to rapidly explore ideas and align teams under ambiguity.

Comfortable moving between strategy and execution, including AI-assisted prototyping, frontend implementation, and systems-level product thinking.

---

# Core Strengths

## Product & UX
- Systems thinking
- UX strategy
- Interaction design
- Information architecture
- Service ecosystem design
- Design systems
- Workflow orchestration
- Onboarding optimization
- AI-assisted UX
- Responsive web and mobile design

## Leadership
- Cross-functional alignment
- Mentorship
- Stakeholder facilitation
- Strategic product thinking
- Workshop facilitation
- Ambiguity navigation
- Executive storytelling
- Design direction

## Technical & AI
- Claude
- Cursor
- Figma AI
- AI-assisted coding
- Functional prototyping
- React
- Frontend development
- Framer code components
- GitHub workflows
- Rapid iterative prototyping

---

# Experience

# G2
Principal Product Experience Designer  
Feb 2024 – May 2026

## Overview
Led strategic product design initiatives across G2’s seller ecosystem, buyer intent platform, onboarding experiences, and design systems modernization efforts.

Worked across highly cross-functional environments spanning product, engineering, brand, marketing, analytics, and executive leadership.

## Key Initiatives

### Elevate Design System
- Spearheaded a cross-functional rebuild of G2’s design system.
- Unified product, brand, and engineering stakeholders around scalable foundations and component architecture.
- Led design consensus, Figma architecture, and implementation planning across multiple squads.
- Improved consistency, accessibility, scalability, and product cohesion.

### G2 Solutions
- Designed and launched the new Seller Solutions framework replacing the underperforming Suites product.
- Enabled SaaS vendors to package and market products by business use case.
- Created inline editing workflows, modular landing pages, customizable CTAs, analytics integrations, and AI-assisted social proof generation.
- Contributed to significant seller engagement increases and early renewal momentum.

### Buyer Intent Ecosystem
- Explored consolidation strategies across fragmented buyer intent products and workflows.
- Designed onboarding, segmentation, integrations, notifications, analytics, and activation experiences.
- Helped define future-state IA and workflow orchestration models.

### AI-First Product Vision
- Co-developed speculative product vision work exploring AI-first, highly self-serve platform experiences.
- Created high-fidelity prototypes, animatics, and executive storytelling artifacts.
- Explored generative UI patterns, onboarding orchestration, adaptive workflows, and conversational interaction models.

## Skills Demonstrated
- Systems thinking
- Product strategy
- Design systems leadership
- Cross-functional facilitation
- AI-assisted prototyping
- Information architecture
- Workflow design
- Executive communication

---

# PetPlate
Head of Product Design  
Oct 2022 – Sep 2023

## Overview
Led product design across customer acquisition, onboarding, subscription management, retention, and ecommerce experiences.

## Contributions
- Led redesign initiatives across onboarding, subscription management, retention, and ecommerce experiences within a rapidly evolving consumer platform.
- Helped mature product design processes, collaboration models, and design system practices across product, engineering, and marketing teams.
- Improved UX consistency and cross-functional alignment through stronger systems thinking, reusable patterns, and more cohesive experience design.
- Mentored designers while helping elevate design quality, strategic thinking, and organizational design maturity.

---

# Wisdom Panel
Director, Product Design
Nov 2019 – Sep 2023

## Overview
Led product and experience design efforts focused on translating complex scientific and genetic health information into approachable consumer experiences.

## Contributions
- Led a major transformation of Wisdom Panel’s digital product experience, modernizing the platform, redesigning core customer journeys, and helping evolve the organization’s design maturity.
- Directed the development of scalable design systems and UX foundations that improved consistency, usability, accessibility, and cross-functional collaboration.
- Translated complex scientific and genetic health information into approachable, trustworthy consumer experiences across onboarding, reporting, and educational workflows.
- Partnered closely with science, engineering, product, and marketing stakeholders to align business goals with emotionally sensitive, human-centered UX.
- Helped shift the organization toward more strategic, systems-oriented product design practices and long-term platform thinking.

---

# Additional Experience
(Add Vacasa and earlier roles later)

---

# Leadership Philosophy

## Player / Coach
Strongly collaborative design leader who enjoys mentoring, facilitating alignment, and helping teams improve without needing heavy organizational hierarchy.

## Systems Thinking
Believes the best product experiences emerge from understanding interconnected systems rather than isolated screens or flows.

## AI Philosophy
Views AI as a collaborative tool for exploration, prototyping, synthesis, and workflow acceleration, while remaining thoughtful about trust, cognitive load, and human oversight.

---

# Preferred Resume Tone

- thoughtful
- strategic
- human
- concise
- systems-oriented
- avoids excessive corporate jargon
- avoids AI hype language
- avoids em dashes
- avoids “20+ years experience”

---

# Tailoring Notes

## Healthcare Roles
Emphasize:
- trust
- systems thinking
- emotionally sensitive UX
- ambiguity navigation
- AI trust boundaries

## AI-Native Roles
Emphasize:
- functional prototyping
- AI-assisted workflows
- frontend fluency
- systems orchestration

## Staff IC Roles
Emphasize:
- strategic ownership
- mentorship
- influence without authority
- cross-functional facilitation

---

# Interview Notes

## Strengths
- Systems-level thinking
- Cross-functional communication
- Strategic ambiguity navigation
- Product ecosystem design
- Human-centered AI thinking

## Watch-outs
- Can over-explain or become overly detailed
- Benefits from concise framing and structured storytelling